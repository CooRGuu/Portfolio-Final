...
// Supprimer le cookie 'userSession' en le mettant à une date d'expiration passée
setcookie('userSession', '', time() - 3600, '/');  // Expire immédiatement

// Créer à nouveau un cookie 'userSession' sécurisé
setcookie('userSession', 'true', time() + 3600, '/', null, true, true);  // Sécurisé, HttpOnly et SameSite
...
if (isset($_SESSION['user_login'])) {
    ...
    // Si l'ID dans l'URL est présent
    if ($userIdInUrl) {
        // Vérifier si l'ID dans l'URL correspond à celui de la session
        if ($userIdInUrl != $_SESSION['user_id']) {
            // Rediriger vers index.php si l'ID ne correspond pas
            header("Location: index.php");
            exit(); // Important d'arrêter l'exécution du script
        }
    }
    ...
    // Si pas de session, assigner null
    $userInfo = null;
}
?>
...
<body>
    ...
    <nav>
        <ul>
            ...
            <li><button class="tab-button" data-tab="check-breach-btn">Vérifier les violations</button></li>
    ...
    <!-- Section pour afficher les résultats de l'API -->

    <section id="check-breach-btn" class="tab-section">
    <h2>Dernières violations de données</h2>
    <div class="container">
        <button id="fetch-breaches-btn">Vérifier les violations récentes</button>
